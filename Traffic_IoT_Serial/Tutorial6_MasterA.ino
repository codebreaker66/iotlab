...
***********************************************************
  IoT Future LAB - Tutorial 6 - vernetze Ampelwelt
                   Master mit Adresse 01 - Version A
  Ver. 1.0
  Date   :  28.03.2022
...
***********************************************************
  ...
  WICHTIG: Es darf immer nur einen Master in einem Ring
           geben
...
uint8_t               sendLen = 5;
uint8_t               recvLen;
uint8_t               ownAdress = 0x01; // Damit ist es ein MASTER
uint16_t              lcount = 0;
...
void loop() {
  ...
  // Pruefen, ob die Zeit abgelaufen ist......
  // und es eine Anforderung gibt
  if (myLastTime + wartezeiten[myState] < millis() && ((myAnforderung) || myState != 0)){
    ...
    if (myState == 1) {
      ...
      sendData[0] = myState;
      sendData[1] = HAUPTRICHTUNGROT;
      sendData[3] = 1;
      iotSerial.send(sendData, sendLen, 0x01);
    }
